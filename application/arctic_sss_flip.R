...
ord <- order(coords_tr[,1]*coords_tr[,2]) # flip the direction 
gg_order <- arctic_utm %>% 
  ggplot() +
...
#   ggsave(plot = gg_order, paste0(path, "plots/arctic_currents_flip", ext),
#          width = 5, height = 3.7)
# }
...
#         paste0(path, "application/arctic_sss_flip_BORAGP.RDS"))
# 
...
#              yess = yess),
#         paste0(path, "application/arctic_sss_flip_BORAGP_processed.RDS"))
# 
# posterior summary
BRGPres <- readRDS(paste0(path, "application/arctic_sss_flip_BORAGP_processed.RDS"))
...
quantile(BRGPres$barrier_m.s$p.theta.samples[burn + thin*1:save,2],             # tau.sq
...
#         paste0(path, "application/arctic_sss_flip_NNGP.RDS"))
# 
...
#         paste0(path, "application/arctic_sss_flip_NNGP_processed.RDS"))
# 
# posterior summary
NNGPres <- readRDS(paste0(path, "application/arctic_sss_flip_NNGP_processed.RDS"))
betaNNGP <- mean(NNGPres$m.s$p.beta.samples[burn + thin*1:save])
thetaNNGP <- colMeans(NNGPres$m.s$p.theta.samples[burn + thin*1:save,])
quantile(NNGPres$m.s$p.theta.samples[burn + thin*1:save,2], 
         probs = c(0.025, 0.5, 0.975))
...
gg_cov <- tibble(rbind(coords_tr_mini, coords_pred_mini),
  ...
  scale_color_scico(palette = "batlow", limits = c(0.101, 1),
                    na.value = "transparent") +
...
# for (ext in extension) {
#   ggsave(plot = gg_together, paste0(path, "plots/arctic_flip_pred", ext),
#          width = 8, height = 6)
# }
