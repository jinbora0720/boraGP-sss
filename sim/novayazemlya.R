...
arctic_avg <- arctic %>% 
  ...
  ungroup()
arctic_avg$ssss <- sqrt(arctic_avg$sss)                                         # to ensure positivity upon back transformation
...
# observed plot
obsplot <- rest_utm %>% ggplot() +
  geom_sf(data = arctic_utm, aes(col = sss)) +
  geom_sf() +
  labs(title = "True sea surface salinity (psu)", color = "")
...
#                  "tau.sq" = tau.sq, "nu" = nu)
# tuning <- list("phi" = 0.001, "sigma.sq" = 0.1, "tau.sq" = 0.005, "nu" = 0)     # nu is fixed 
...
#                  n.omp.threads = 10, verbose = FALSE)
#   
#   #---------#
#   # Results #
#   #---------#
...
#   orgystarBRGP <- rowMeans(barrier_p.s$p.y.0^2)
#   
#   ## NNGP
#   ystarNNGP <- rowMeans(p.s$p.y.0)
#   yquantNNGP <- apply(p.s$p.y.0, 1, 
#                       function(x) quantile(x, probs = c(0.025, 0.975)))
#   orgystarNNGP <- rowMeans(p.s$p.y.0^2)
#   
#   ## inla
...
#   orgy_inla <- sapply(inlares$marginals.fitted.values[index.pred],
#                       function(s) inla.emarginal(function(x) x^2, s))
#   
#   # prediction summary
...
# plot
gg_4 <- res$longdata_list[[19]][c(1:n_tr, (n + n_pred) + 1:n, 
...
## near Novaya Zemlya
gg_novaya <- res$longdata_nb_list[[19]] %>% 
...
for (i in 1:length(pt)) {
  ...
  plot_neighbor[[i]] <- rest_utm %>% ggplot() + geom_sf() +
    geom_point(data = coords_all[ptnn, ], 
               aes(easting, northing, col = "NNGP", 
                   size = "NNGP", shape = "NNGP")) +
    geom_point(data = coords_all[barrier_ptnn,], 
            aes(easting, northing, 
                col = "BORA-GP", size = "BORA-GP", shape = "BORA-GP")) +
    geom_point(data = coords_all[n_tr+pt[i],], aes(easting, northing), 
               col="black", size = 2) +
    labs(x = "", y = "", color = "", shape = "",
         size = "", title = "15 neighbors") +
    scale_color_manual(label = c("BORA-GP", "NNGP"),
                       values = c("#DC3220", "#888888")) +
    scale_size_manual(label = c("BORA-GP", "NNGP"),
                      values = c(2, 3)) +
    scale_shape_manual(label = c("BORA-GP", "NNGP"),
                       values = c(17, 19)) +
    theme(legend.position = c(0.85,0.9),
          plot.margin = margin(t = 0, l = -3, r = 1, b = -1),
  ...
  nnplot_cov <- data.frame(coords_all[1:n,], Cor = NNGP_cov) %>%
    filter(Cor > 0.2) %>% 
    ggplot() +
    geom_point(aes(easting, northing, col = Cor)) +
    scale_color_scico(palette = "batlow", limits = c(0.2,1)) + # lapaz
    geom_point(data = coords_all[n_tr+pt[i],], aes(easting, northing), 
               col="black", size = 2) +
    geom_sf(data = rest_utm) +
    labs(title = "NNGP correlation", x = "", y = "") +
    theme(legend.position = "right",
          plot.margin = margin(t = 0, l = -3, r = 1, b = -1),
          legend.margin = margin(b = 0, r = 0, t = 0, l = 0),
          plot.title = element_text(hjust = 0.5))
    
  brplot_cov <- data.frame(coords_all[1:n,], Cor = boraGP_cov) %>%
    filter(Cor > 0.2) %>% 
    ggplot() +
    geom_point(aes(easting, northing, col = Cor)) +
    scale_color_scico(palette = "batlow", limits = c(0.2,1)) + # batlow
    geom_point(data = coords_all[n_tr+pt[i],], aes(easting, northing), 
               col="black", size = 2) +
    geom_sf(data = rest_utm) +
    labs(title = "BORA-GP correlation", x = "", y = "") +
    theme(legend.position = "right",
          plot.margin = margin(t = 0, l = -3, r = 1, b = -1),
          legend.margin = margin(b = 0, r = 0, t = 0, l = 0),
          plot.title = element_text(hjust = 0.5))

  gg_cov[[i]] <- ggpubr::ggarrange(brplot_cov, nnplot_cov,
                              common.legend = TRUE, legend = "right", nrow = 1)
}
gg_covall <- ggpubr::ggarrange(plot_neighbor[[1]],gg_cov[[1]],
                               plot_neighbor[[2]],gg_cov[[2]],
                               common.legend = F, nrow = 2, ncol = 2, 
                               widths = c(0.95/3,2.05/3))
# for (ext in extension) {
